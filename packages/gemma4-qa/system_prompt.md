...
1. list_directory(path) — lists the documents as a CATALOG: one line per file with the document's title and its main section headings (the first few; "+N more" means the document has more sections than shown). Call it first — the catalog is how you pick the right document.
